...
# 게시판 댓글 많이 달린 순위 START_RANK 부터 LAST_RANK 까지 탐색
...
comments = [] # 최종 수집될 댓글이 저장된다.

# 모든 URL들을 순회하며, 댓글을 수집한다.
for url in urls:
  date = url.split("=")[-1]
  year = date[:4] # 연도
  month = date[4:6] # 월
  ...
  browser.get(url) # 해당 URL로 이동한다.

  # 이동한 URL(사이트)에서 게시판 랭킹 순위 START_LANK 부터 LAST_RANK까지 탐색한다.
  for i in range(SEARCHING_START_RANK, SEARCHING_LAST_RANK + 1):
    try:
      # 아래의 CSS 태그에 해당하는 기사를 클릭하여 사이트를 이동한다.
      if i <= 5:
